...
PRIMERS = {
    ...
    "最大公约数": "手写欧几里得算法求 gcd；同时检查 0 值、符号和 lcm 乘法溢出。",
...
- 使用 `cin/cout` 时，在 `main` 开头关闭同步并解除绑定；普通换行优先写
  `'\n'`，只有确实需要立即刷新缓冲区时才用 `endl`。
- `gets` 已从当前环境删除。C++ 读取整行使用 `getline(cin, s)`；混用
  `cin >>` 与 `getline` 时，先处理上一轮留下的换行。
- 提交时只使用标准输入输出，不依赖本地文件、绝对路径或交互式提示语。
